...
변수의 선언
		자료형 변수명 = 초기값;
		자료형 변수명;  --> 선언만 하면 garbage value가 들어감.

		예) int x = 10;
		x라는 이름의 정수형 저장공간이 할당되고 그 안에 10이 들어감.
...
===========================================================================================
2차원 배열 : 배열 안에 배열 --> 값에 접근하기 위해 2번 접근해야 됨.
	1차원 배열을 여러개 선언하면, 관리하기 어려움.
	2차원 배열을  한 번만 선언해서 관리.

	※ 2차원 배열부터는 메모리 낭비가 심하므로 선호하지 않음.
...
super() : 부모 생성자
	자식 클래스 타입의 객체는 부모 필드에 접근할 수 있음.
	하지만 자식 생성자만 호출하기 때문에, 자식 필드만 메모리에 할당됨.
	그렇기 때문에, B타입의 객체로 A필드인 부모 필드에 접근할 수 없어야 정상.
	사실 자식 생성자에서 항상 부모 생성자를 호출하기 때문에 자식 생성자 호출 시
	부모와 자식 필드 모두 메모리에 할당됨. 이 때 부모 생성자를 호출하는 방법은
	super()를 사용하는 것. 만약 작성하지 않더라고 컴파일러가 자동으로 작성.
...
인터페이스(틀) != 클래스
	추상 클래스를 고도화시킨 문법, 상수와 추상 메소드만 존재함.
	구현은 지정한 클래스에서 진행하고, 인터페이스를 다른 클래스에 지정할 때에는
	implements 키워드를 사용함.

추상 클래스와 인터페이스 간의 관계
	인터페이스를 클래스에 바로 지정하면 모든 메소드에 강제성이 부여되기 때문에
	전부 다 구현해야 함. 하지만 일반적인 상황에서는 필요한 메소드를 골라서 재정의함.
	따라서 인터페이스를 직접 지정하지 않고 다른 클래스에 지정한 후 구현해 놓는다면,
	강제성이 소멸되고 이로 인해 골라서 재정의할 수 있게 됨.
	이 때 중간에서 강제성을 없애주는 클래스를 추상 클래스로 선언하기로 하며,
	추상 클래스 이름 뒤에는 Adapter를 붙여서 목적을 알려줌.


내부 클래스(Inner Class) - (모바일 개발에서 많이 사용)
...
예외 처리
	...
	아예 막을 수 없는 상황도 발생. 컴퓨터에 이상이 생길 정도로 심각한 오류는 에러라고 표현하고
	프로그램에 이상이 생긴 것은 예외라고 부름. 하지만 정해진 용어는 없으니 보통 "오류"라고 부름.
	프로그램이 강제 종료되는 것을 막는 게 예외 처리의 목적.
...
테스트용 앱 비밀번호 : oxmr bzet brmj btbk
...
================================================================================================
Object 클래스
	최상위 부모 클래스로서, 모든 클래스는 자동으로 Object를 상속받음.
...
동기화(Synchronized)
  ...
  파일 입출력
	...
	Buffer를 사용한 입출력을 권장함. Buffer를 사용하면 일괄처리가 가능해짐.
